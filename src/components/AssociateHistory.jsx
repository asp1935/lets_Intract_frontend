...
const AssociateHistory = () => {
  ...
  const { data: historyData } = useHistory('associate')
  ...
  const handleDownloadAll = () => {
    ...
    const tableColumn = ["#", "Name", "Mobile", "Email", "Payment Date", "Payment Via", "Amount"];
    ...
    doc.save("all_associate_payouts.pdf");
  };

  return (
    <div className="Form payment_history p-6 bg-gradient-to-r from-purple-50 to-pink-50 shadow-lg rounded-lg mt-10">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-[#640D5F]" style={{ textShadow: "3px 3px 10px rgba(100, 13, 95, 0.7)" }}>Associate Payment History</h2>
            ...
            {filteredHistory.length > 0 ? (
              filteredHistory.map((entry, index) => (
                <tr key={index} className="hover:bg-gray-50 transition-colors duration-200">
                  ...
                  <td className="p-4 border-b border-gray-200">{entry.mobile}</td>
