...
const AssociatePayout = () => {
  ...
  const { data: associateData } = useAssociatePayout();
  ...
  const handleConfirmPayout = () => {
    ...
    if (!selectedUser || !selectedUser._id) {
      dispatch(showToast({ message: "Select Valid Associate", type: 'error' }))
      return
    }
    ...
    associatePayment.mutate(({ associateId: selectedUser._id, paymentData: payment }), {
      onSuccess: () => {
        setIsModalOpen(false);
        setSelectedUser(null);
        setPayment({
          ...
          <div className="p-5 mt-10">
            <div className="bg-white p-6 rounded-lg w-[500px] max-h-[80vh] flex flex-col" style={{ fontSize: "16px", boxShadow: "rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -3px, rgba(0, 0, 0, 0.2) 0px -3px 0px inset" }}>
              ...
              <div className="flex-shrink-0">
                ...
                <p className="mb-2 flex justify-between mx-1">
                  <strong>Total Commission:</strong> {selectedUser?.amount}
                </p>
              </div>
